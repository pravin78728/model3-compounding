"""
add_forward_dates.py
Adds Jun 2025, Dec 2025, Jun 2026 rebalance dates to model3_training_data.
Uses same signal computation logic as build_historical_features.py.
"""
...
for symbol, year, sales, net_profit, eps, borrowings, eq_cap, reserves, roce in cur.fetchall():
    ...
    financials.setdefault(symbol, []).append({
        ...
        'roe': roe, 'de': de, 'roce': float(roce) if roce else None,
    })
...
def s1_roe_trend(symbol, rebal_date):
    ...
    return round(min(100, max(0, sum(roes)/len(roes)*2 + (roes[0]-roes[-1])*2)), 2)
...
def s2_revenue_cagr(symbol, rebal_date):
    ...
    return round(min(100, max(0, ((latest/oldest)**(1/n)-1)*200)), 2)
...
def s8_peg(symbol, rebal_date):
    ...
    return round(min(100, max(0, (2-pe/growth)*50)), 2)
...
def s11_roce(symbol, rebal_date):
    rows = sorted([r for r in financials.get(symbol, [])
                  ...
                  key=lambda x: x['year'], reverse=True)[:3]
    if not rows: return None
    return round(min(100, max(0, sum(r['roce'] for r in rows)/len(rows)*3)), 2)
...
def s12_eps_cagr(symbol, rebal_date):
    ...
    n = rows[0]['year'] - rows[-1]['year']
    if oldest <= 0 or n == 0: return None
    return round(min(100, max(0, ((latest/oldest)**(1/n)-1)*150)), 2)
...
for rebal_date in rebalance_dates:
    ...
    cur.execute("SELECT COUNT(*) FROM model3_training_data WHERE rebalance_date = %s", (rebal_date,))
    if cur.fetchone()[0] > 0:
        print(f"  ⚠ {rebal_date}: already exists, skipping")
        continue

    universe = get_universe(rebal_date)
    if not universe:
        print(f"  ⚠ {rebal_date}: no universe found")
        continue
    ...
    if batch:
        ...
        """, batch)
        conn.commit()
        print(f"  ✓ {rebal_date}: {len(batch)} stocks | s14={s14} | fwd_available={sum(1 for r in batch if r[17] is not None)}")
        total_rows += len(batch)

print(f"\n✓ Done. {total_rows} rows added.")
conn.close()
