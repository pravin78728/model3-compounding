"""
backtest_with_filter.py — Baseline: RF + governance filter, top 10
This is the best model so far: 30.5% CAGR full period, 53.6% validation.
"""
...
FEATURES = [
    ...
    's5_promoter_trend', 's6_earnings_consist',
...
for rebal_date in rebal_dates:
    ...
    excluded, _ = get_exclusions(gov_conn, rd_date)
    ...
    if stock_returns:
        ...
        portfolio_log.append({
            'date': rebal_date,
            'excluded': before - len(period_df),
            'n_stocks': len(stock_returns),
            'portfolio_return': round(port_ret*100, 1),
            'top_picks': selected
        })
        print(f"  {rebal_date}: excluded={before-len(period_df)} | n={len(selected)} | return={port_ret*100:.1f}%")
...
def cagr(r, ppyr=2):
    return float((1+r).prod())**(1/(len(r)/ppyr))-1

def sharpe(r, ppyr=2, rf=0.065):
    ex = r-rf/ppyr
    return float((ex.mean()/ex.std())*np.sqrt(ppyr))

def max_dd(r):
    eq=(1+r).cumprod()
    return float(((eq-eq.cummax())/eq.cummax()).min())

pc=cagr(port_series); ps=sharpe(port_series); pd_=max_dd(port_series)
vp=port_series[port_series.index>='2020-01-01']
vpc=cagr(vp); vps=sharpe(vp); vpd=max_dd(vp)

print(f"\n══════════════════════════════════════════")
print(f"  MODEL 3 — BASELINE: RF + GOVERNANCE")
print(f"══════════════════════════════════════════")
